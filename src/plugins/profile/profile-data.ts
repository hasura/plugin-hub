...
export const profileData = (data: ObjMap<unknown>): Analysis => {
  ...
  const validValues = table
    ...
    .filter(<any>aq.escape((d: ValueObservation) => typeof d.value !== 'number' || !isNaN(d.value)))
  ...
  for (const entry of keys) {
    ...
    const values = validValues.filter(<any>aq.escape((d: ValueObservation) => d.datasetKey === datasetKey && d.columnKey === columnKey)).array('value')
  ...
  return [...typeMap.entries()].reduce<Analysis>((acc, [[datasetKey, columnKey], scalarType]): Analysis => {
    ...
    acc[datasetKey][columnKey] = analyze[scalarType]?.(table.filter(<any>aq.escape((d: ValueObservation) => d.datasetKey === datasetKey && d.columnKey === columnKey)))
    return acc
  }, {})
}
